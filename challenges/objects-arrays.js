...
const universitiesSorted = universities.sort(); //used to sort universites, i could of console.log(universities.sort()) also...
...
const uni = [];
for (let i = 0; i < graduates.length; i++) {
  (graduates[i].university.includes('Uni'))? uni.push(graduates[i].university): null;
}
console.log(uni);
...
*/
const largerPopulation = zooAnimals.filter(animal => animal.population < 5); //return all info of the animal with a pop less than 5
...
const zooPops = zooAnimals.map(animal => animal.population) //mapped the population into individual array to use in reduce.

const populationTotal = zooPops.reduce((acc, nextNum) => {  //reduced the zooPops to reduce number into one...
  return acc + nextNum
}, 0);
